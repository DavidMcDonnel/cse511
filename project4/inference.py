...
class ExactInference(InferenceModule):
  ...
  def elapseTime(self, gameState):
    ...
    for prevPos in self.legalPositions:
        dist = self.getPositionDistribution(self.setGhostPosition(gameState,prevPos))
        for pos,prob in dist.items():
            allPossible[pos]+=prob*self.beliefs[prevPos]
    self.beliefs=allPossible
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance==None:
        ...
        for pos in self.legalPositions:
            ...
            allPossible[pos]+=emissionModel[dist]*oldBelief[pos]
        if not any(allPossible.values()):
            self.initializeUniformly(gameState)
        else:
            tmp = []
            for _ in range(0,self.numParticles):
                tmp.append(util.sample(allPossible))
            self.particles=tmp
  ...
  def elapseTime(self, gameState):
    ...
    for pos in self.particles:
        newPos = self.getPositionDistribution(self.setGhostPosition(gameState,pos))
        particles.append(util.sample(newPos))
  ...
  def initializeParticles(self):
    ...
    for _ in range(self.numParticles):
      ...
      for _ in range(self.numGhosts):
        particle.append(random.choice(self.legalPositions))
    ...
    for i, particle in enumerate(self.particles):
      ...
      for ghost in range(self.numGhosts):
        if noisyDistances[ghost] is None:
          ...
          particle[ghost]=self.getJailPosition(ghost)
          particle = tuple(particle)
        else:
          dist = util.manhattanDistance(particle[ghost],pacmanPosition)
          p*=emissionModels[ghost][dist]
      pot[particle]+=p
    if not any(pot.values()):
      self.initializeParticles()
